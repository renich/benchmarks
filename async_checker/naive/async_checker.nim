...
proc runTask(id: int): TaskResult =
...
proc main() =
  ...
  var errCount = 0
  var totalLat = 0'i64
  var latencies = newSeq[int](tasks)
  ...
  latencies.sort()
  let p50 = latencies[int(float(tasks) * 0.50)]
  let p95 = latencies[int(float(tasks) * 0.95)]
  let p99 = latencies[int(float(tasks) * 0.99)]
