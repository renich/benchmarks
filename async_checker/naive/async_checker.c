...
int main(void) {
    ...
    int *latencies = (int *)malloc(NUM_TASKS * sizeof(int));
    int lat_idx = 0;

    TaskResult res;
    while (q_pop(&res_q, &res)) {
        latencies[lat_idx++] = res.latency;
    ...
    qsort(latencies, NUM_TASKS, sizeof(int), cmp_int);
    int p50 = latencies[(int)(NUM_TASKS * 0.50)];
    int p95 = latencies[(int)(NUM_TASKS * 0.95)];
    int p99 = latencies[(int)(NUM_TASKS * 0.99)];
