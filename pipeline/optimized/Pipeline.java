...
public class Pipeline {
    ...
    public static void main(String[] args) throws Exception {
        ...
        byte[] prefix = "task:item:".getBytes(StandardCharsets.US_ASCII);
            ...
            workerThreads[w] = new Thread(() -> {
                try {
                    ...
                    while (true) {
                        ...
                        int numLen = 32 - idx;
                        byte[] payload = new byte[prefix.length + numLen];
                        System.arraycopy(prefix, 0, payload, 0, prefix.length);
                        System.arraycopy(buf, idx, payload, prefix.length, numLen);

                        long chk = fnv1a(payload, payload.length);
